...
describe('Update', () => {
  ...
  describe('post', () => {
    it('should send the correct json response', async () => {
      ...
      expect(postQueue.addPostJob).toHaveBeenCalledWith(UPDATE_POST_IN_DB, { key: `${postMockData._id}`, value: updatedPost });
  ...
  describe('postWithImage', () => {
    it('should send the correct json response if imageId and imgVersion exist', async () => {
      ...
      updatedPostWithImage.image = 'data:image/jpeg;base64,/9j/4AAQSkZJRgABAQAAAQABAAD/';
